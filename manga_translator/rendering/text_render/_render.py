...
def _scale_parts_x(parts, scale_x: float):
    ...
    for layer in parts:
        if layer is None:
            scaled.append(None)
            continue
        scaled_w = max(1, int(math.ceil(layer.shape[1] * scale_x)))
        scaled.append(cv2.resize(layer, (scaled_w, layer.shape[0]), interpolation=cv2.INTER_AREA))
    return tuple(scaled)
...
def _text_layer_parts(
    ...
    无墨迹返回 None。
    """
    border_size = max(int(round(font_size * stroke_ratio)), 1) if stroke_ratio > 0 else 0
    ...
    return (
        _finish_layer(_rgba_for_paint_part(text_alpha, border_alpha, fill, stroke, 'all'), style, font_size, 'effects'),
        _finish_layer(_rgba_for_paint_part(text_alpha, border_alpha, fill, stroke, 'stroke'), style, font_size, 'body'),
        _finish_layer(_rgba_for_paint_part(text_alpha, border_alpha, fill, stroke, 'fill'), style, font_size, 'body'),
    )
...
def _vertical_char_parts(plan: VerticalCharPlan):
    ...
    combined, _, _ = _glyph_pair_rgba(bitmap, stroke_bitmap, plan.fill, plan.stroke, 'all')
    stroke_base, _, _ = _glyph_pair_rgba(bitmap, stroke_bitmap, plan.fill, plan.stroke, 'stroke')
    fill_base, _, _ = _glyph_pair_rgba(bitmap, stroke_bitmap, plan.fill, plan.stroke, 'fill')
    return (
        ...
        for run in layout.runs:
            ...
            if run.has_ink and run.main_rect is not None:
                ...
                if parts is not None:
                    glyph_items.append((
                        parts,
                        int(round(cursor_x + run.main_rect.x)),
                        int(round(baseline_y + run.main_rect.y)),
                    ))
            ruby = run.ruby
            if ruby is not None:
        ...
        for emphasis in layout.emphasis_plans:
            ...
            for main_center in emphasis.main_centers:
                fill_extras.append((
                    ...
                    (body_right + emphasis.cross_center, line_origin_y + main_center,
                     emphasis.radius, dot_color),
                ))
