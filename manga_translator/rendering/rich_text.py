...
class StrokeStyle:
    ...
    @classmethod
    def from_dict(cls, value: dict | None) -> Optional["StrokeStyle"]:
        if value is None:
            return None
        ...
        if not value:
            return None
        return cls(
            color=value.get("color"), width=_safe_float(value.get("width"), None)
        )
...
class GlowStyle:
    ...
    @classmethod
    def from_dict(cls, value: dict | None) -> Optional["GlowStyle"]:
        if value is None:
            return None
...
class TextTransform:
    ...
    @classmethod
    def from_dict(cls, value: dict | None) -> "TextTransform":
        if value is None:
            value = {}
        ...
        scale_x = float(1.0 if value.get("scaleX") is None else value["scaleX"])
        scale_y = float(1.0 if value.get("scaleY") is None else value["scaleY"])
        if (
            ...
            raise ValueError(
                "style.transform scaleX/scaleY must be finite and greater than 0"
            )
        return cls(
        ...
        return _drop_none(
            {
                ...
                "scaleX": None if self.scale_x == 1.0 else self.scale_x,
                "scaleY": None if self.scale_y == 1.0 else self.scale_y,
            }
        )
...
class TextStyle:
    ...
    outer_stroke: StrokeStyle | None = None
    glow: GlowStyle | None = None
    emphasis: bool = False
    no_tcy: bool = False
    vertical_advance: Literal["half", "full"] | None = None
    kerning: float = 0.0
    ...
    @classmethod
    def from_dict(cls, value: dict | None) -> "TextStyle":
        if value is None:
            value = {}
        ...
        return _drop_none(
            {
                ...
                "outerStroke": self.outer_stroke.to_dict()
                if self.outer_stroke
                else None,
...
class RubyRun:
    ...
    def from_dict(cls, value: dict) -> "RubyRun":
        ...
        return cls(
            base=[
                TextRun.from_dict(item)
                for item in _required_list(value, "base", "ruby inline")
            ],
            text=[
                TextRun.from_dict(item)
                for item in _required_list(value, "text", "ruby inline")
            ],
        )
    ...
    def to_dict(self) -> dict:
        return {
            "type": "ruby",
            "base": [item.to_dict() for item in self.base],
...
class TcyRun:
    ...
    def from_dict(cls, value: dict) -> "TcyRun":
        ...
        return cls(
            content=[
                TextRun.from_dict(item)
                for item in _required_list(value, "content", "tcy inline")
            ]
        )
...
InlineNode = TextRun | RubyRun | TcyRun
    ...
    def spans(self) -> list[RenderSpan]:
        ...
        for inline in self.inlines:
            if isinstance(inline, TextRun):
                ...
                spans.append(
                    RenderSpan(
                        base_text, style, ruby=inline.text if ruby_text else None
                    )
                )
            elif isinstance(inline, TcyRun):
                text = "".join(run.text for run in inline.content)
                style = (
                    inline.content[0].style.copy() if inline.content else TextStyle()
                )
...
class RichTextDocument:
    ...
    def from_dict(cls, value: dict) -> "RichTextDocument":
        ...
        return cls(
            blocks=[
                Paragraph.from_dict(block)
                for block in _required_list(value, "blocks", "rich text document")
            ]
        )
...
def is_rich_text_document(value: Any) -> bool:
    return isinstance(value, RichTextDocument) or (
        isinstance(value, dict) and value.get("format") == RICH_TEXT_FORMAT
    )
...
def ensure_rich_text_document(value: Any) -> RichTextDocument:
    ...
    raise TypeError(
        f"Expected RichTextDocument or {RICH_TEXT_FORMAT} dict, got {type(value).__name__}"
    )
...
def legacy_line_breaks_to_document(
    text: str, style: TextStyle | None = None
) -> RichTextDocument:
...
def _parse_vertical_advance(value) -> Literal["half", "full"] | None:
