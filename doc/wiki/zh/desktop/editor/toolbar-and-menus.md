...
进入编辑器后，顶部有一条固定的横向工具栏。它把高频操作收进三个单级下拉菜单（“菜单”、“显示模式”、“排列”），并保留“适应窗口”和“原图不透明度”两个常驻控件。这里说明这三个菜单的展开方式、每个菜单项的去向，以及五个编辑开关的存储与持久化。
...
1. 打开“菜单”：显示“导出图片”（代码追加 `(Ctrl+Q)` 提示）、撤销/重做（提示 `Ctrl+Z` / `Ctrl+Y`）、放大/缩小（“放大 (+)”“缩小 (-)”），以及五个带勾选标记的编辑开关。

#### 五个编辑开关

“菜单”下拉里的五个编辑开关都带勾选标记，含义与默认值如下：
