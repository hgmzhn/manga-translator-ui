...
本页说明桌面“翻译界面”中待处理输入的添加、展示和移除：单个图片文件、文件夹、拖放路径以及受支持的压缩包/文档。文件列表只负责收集输入源、建立缩略图树和提供选择状态；输出目录、工作流选择、开始按钮和任务进度分别见[输出目录与工作流](./output-directory-and-workflow.md)及[进度、停止与任务状态](./progress-stop-and-task-state.md)。
