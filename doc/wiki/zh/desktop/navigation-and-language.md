...
这里说明桌面 Qt 窗口如何在翻译工作区、设置、API 管理、提示词、规则、批量管理和编辑器之间切换，并说明主题与界面语言的配置边界。文件列表、工作流按钮、编辑器内部工具和各二级弹窗分别由对应功能页负责。
