...
本页覆盖翻译页的九种工作流，以及“设置 → 模式相关 → 替换翻译”中的模板对齐参数。它不重复超分/上色字段（见[超分与上色](./upscale-and-colorization.md)）或各阶段通用参数。工作流是 `cli` 分支；模板对齐是 `render` 分支。GUI 选择模式时八个工作流标志先全部清零，因此一次选择只有一个模式；手工叠加多个标志不属于稳定契约。
