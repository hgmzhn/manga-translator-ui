...
“导出翻译”默认只把本地已有工程 JSON 中的译文单独导出到 `translations/` 副文件，不重新读取图片做检测或 OCR，不调用翻译 API，也不回写工程 JSON。这样导出操作不会覆盖用户在 JSON 中修改好的译文。该默认行为由“设置 → 模式相关 → 文本导出 → 仅从本地 JSON 导出文本”控制；关闭开关才恢复旧的图片检测、OCR、翻译和 JSON 写入流程。
...
- 默认输入：每张图片已有的 `manga_translator_work/json/<stem>_translations.json` 和可读取的导出模板；图片文件只用于定位同名工程 JSON，不会被打开解码。
- 默认执行：读取 JSON `regions[].translation`，移除 `[BR]`，按模板写译文副文件。
- 默认跳过：图片读取、上色、超分、检测、OCR、翻译、蒙版、修复、渲染、主图保存和工程 JSON 回写。
- 默认输出：`manga_translator_work/translations/<stem>_translated.<模板扩展名>`；原工程 JSON 保持不变。
...
默认开启 `cli.export_from_local_json` 时，`translate_batch()` 在图片物化之前直接进入本地 JSON 导出分支；缺少 JSON 时该图片明确失败，不回退 OCR。关闭开关后才进入下图所示的旧流水线。
...
- 默认本地导出只读工程 JSON，不写 `skip_font_scaling`、蒙版、模型信息或任何其它字段。
- 用户编辑后的 `translation`、富文本、画笔/印章图层及未知扩展字段均原样保留。
- 关闭 `cli.export_from_local_json` 后，旧流程才会重新生成并写回上述运行字段。

### 与并发和互斥的关系

- `batch_concurrent` 不兼容；本地 JSON 导出本身逐项处理且不会加载模型。
- GUI 切换时八个工作流字段互斥；`cli.export_from_local_json` 不是工作流字段，而是同时控制两种文本导出的独立开关。
- 关闭本地 JSON 开关后，才按旧流程执行条件上色、超分、检测、OCR 和翻译，并可能产生相应资源与 API 成本。

## 输入、输出与限制

- 模板依赖：模板不可解析时译文副文件不产出，工程 JSON仍保持不变。
- 工程 JSON 依赖：找不到或无法读取时该图片失败，不自动回退到 OCR，避免意外重建并覆盖数据。
- `cli.overwrite=false`：GUI 开始前跳过译文副文件已存在的图片。
- `cli.save_text`：导出翻译不依赖它；默认本地导出只读 JSON，不写 JSON。
- 只有关闭 `cli.export_from_local_json` 后，旧流程才会使用上色、超分、检测、OCR、翻译模型和 API。
