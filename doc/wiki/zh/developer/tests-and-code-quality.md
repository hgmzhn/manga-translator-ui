...
- 默认 `uv sync` 安装 `gpu` + `packaging` + `test`（NVIDIA CUDA 13.0、PyInstaller 与测试工具）；`cuda12.6` 分支构建备用 CUDA 12.6 环境。安装器禁用默认组，因此不会检测测试依赖。`cpu`、`gpu`、`amd`、`metal` 四组互斥，见 `pyproject.toml` 的 `[tool.uv] conflicts`。
