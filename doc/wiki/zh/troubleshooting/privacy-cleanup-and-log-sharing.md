...
当你要释放磁盘空间、删除含个人内容的运行数据，或把日志和调试产物发给别人排查问题时，本页说明数据清理的对象与顺序、脱敏规则以及日志分享的注意事项。逐类调试产物的生成阶段和阅读顺序见[如何阅读与分享一次调试运行](../debugging/how-to-read-and-share-a-debug-run.md)；按安装形态完整卸载见[卸载与数据清理](../install/uninstall-and-data-cleanup.md)；Web 管理界面的完整操作见[管理界面](../web/administrator-interface.md)。
