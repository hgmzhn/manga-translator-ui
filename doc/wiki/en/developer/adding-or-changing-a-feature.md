...
## Three-layer defaults {#three-layer-defaults}

The same config key has three separate default sources. Do not merge them when adding a field:

| Source | Defining file | Notes |
| --- | --- | --- |
| Core `Config()` | `manga_translator/config.py` | Pydantic defaults of the backend `Config` model; also read by CLI and Web |
| Qt `AppSettings()` | `desktop_qt_ui/core/config_models.py` | Defaults of the desktop settings model |
| Release `config-example.json` | `config/config-example.json` | Release template; the desktop app falls back to it and syncs the user config against it |
...
| Config-chain source | Complete | Statically checked `config_models.py`, `config.py`, `config_service.py`, and consumers |
| i18n keys and actual values | Complete | Every table row taken from `i18n.generated.json` and the two locales |
| Settings-catalog tooling | Complete | Statically checked inputs/outputs of `build-i18n-catalog.mjs` and `build-settings-catalog.py` |
| Sanitized runtime verification | Deferred | No real `.env`, user `config.json`, API key/token, username, or private path was read |
