...
- `render.disable_auto_wrap` is displayed as “AI 断句 / AI Line Breaking”. It drives both the translator-side line-break prompt and the renderer-side `[BR]` forced-wrap semantics; renderer auto wrapping itself is covered by the typesetting page.
- `OPENAI_GLOSSARY_PATH` (displayed as “术语表路径 / Glossary Path”) is a legacy environment-variable-backed glossary path and is separate from the location where `extract_glossary` writes terms (the custom prompt file).
...
2. Toggle “Auto Extract Glossary” (`Auto Extract Glossary`). When enabled, the description panel shows “Auto-extract proper nouns (names, places) from translations to ensure consistency in long manga series.”
3. Toggle “Enable Streaming” (`Enable Streaming`). The description panel explains the difference between streaming and non-streaming requests.
4. Glossary extraction requires a parseable prompt file selected under “Custom Prompt” (`Custom Prompt`); without one, the switch does not produce the extraction branch.
5. Open “Settings” → “Typesetting” (`Typesetting`) and toggle “AI Line Breaking” (`AI Line Breaking`) to enable the AI line-break prompt.

### Inspect the glossary in Prompt Preview

Open “Prompt Management” (`Prompt Management`), select a custom prompt file, and click “Prompt Preview” (`Prompt Preview`). If the file contains a `glossary` key, the preview shows a “Glossary” (`术语词典`) section with a total entry count and per-category tabs for Person / Location / Org / Item / Skill / Creature; when empty it shows “No glossary entries” (`没有术语条目`).
...
- Diagram: glossary-extraction switch branch (below).
...
- Diagram: streaming switch and fallback branch (below).
...
- Diagram: line-break switch branch (below).
...
| UI layout and calls | Complete | Statically checked settings layout, dynamic settings, and prompt-preview calls |
| `en_US` / `zh_CN` actual locales | Complete | The table records key, actual English, and actual Simplified Chinese values |
| Glossary/streaming/line-break runtime chain | Complete | Statically checked prompt assembly, the unified streaming transport, term write-back, and `[BR]` checking |
