...
| Export Translation | Existing project JSON and available template by default; main image when local-JSON export is disabled | By default only `<stem>_translated.<template-format>`; project JSON unchanged; no main image | Read JSON `translation` and export through template | With `export_from_local_json=true`, skips image loading, detection, OCR, API translation, and JSON write-back; disabling it restores the legacy flow |
| Export Original Text | Existing project JSON and available template by default; main image when local-JSON export is disabled | By default only `<stem>_original.<template-format>`; project JSON unchanged; no main image | Read JSON `text` and export through template | With `export_from_local_json=true`, skips image loading, detection, OCR, and JSON write-back; disabling it restores the legacy flow |
...
- Both text exports require an existing project JSON and readable template by default. Missing JSON fails explicitly without OCR fallback; disable `cli.export_from_local_json` to restore legacy detection/OCR from the main image.
- `cli.overwrite` checks existing TXT, sidecar, or main output files per mode before starting.
- `cli.save_text` defaults to `true` in Qt/release configuration and affects normal-mode project writes. It remains part of the Export Original Text workflow flags, but default local export never writes JSON back.
- Selected stage parameters determine model, VRAM, network, and API costs; default local text export does not enter those stages.
