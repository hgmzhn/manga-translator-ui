...
| Export Translation | Existing project JSON and readable template by default; main image when local-JSON export is disabled | By default skips image loading, colorization, upscaling, detection, OCR, translation, mask, inpainting, rendering, and JSON write-back | Only `translations/<stem>_translated.<template-format>`; project JSON unchanged | [Export Translation](../workflows/export-translation.md) |
| Export Original Text | Existing project JSON and readable template by default; main image when local-JSON export is disabled | By default skips image loading, colorization, upscaling, detection, OCR, translation, mask, inpainting, rendering, and JSON write-back | Only `originals/<stem>_original.<template-format>`; project JSON unchanged | [Export Original Text](../workflows/export-original.md) |
...
| `cli.export_from_local_json` | Defaults to `true`; both text exports read local project JSON without image/OCR/API stages or JSON write-back; disabling it restores the legacy flow |
...
| `detector.import_yolo_labels` | Affects masks only in the legacy export flow after local-JSON export is disabled; Import Translation uses detection when JSON has no mask |
