...
| --- | --- | --- |
| BLUEPRINT, PAGE_GUIDELINES, TODO | Done | read the blueprint reference-index boundary, page guidelines, and TODO sections 1.3/5.16 |
| i18n three-column evidence | Done | all nine modes and start buttons record the call key, actual English value, and actual Simplified Chinese value |
| Source and research material | Done | verified against `workflow-matrix-source-evidence.md`, `phase0-page-coverage-matrix.md`, and the listed UI, i18n, controller, and core sources |
| Route mirror / source evidence checks | Done | ran `node scripts/verify-route-mirror.mjs .` and `node scripts/verify-source-evidence.mjs .` from `doc/wiki`; both passed |
| Sanitized runtime verification | Deferred | GUI buttons/prompts, overwrite dialogs, files retained after cancellation, and the JSON-only deletion direction of the nine modes await runtime verification per the research material |
| Nine-workflow difference Mermaid | Deferred | tracked as a future visual task in TODO section 7; this page does not fabricate a diagram early |