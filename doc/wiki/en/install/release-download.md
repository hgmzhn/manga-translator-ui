...
3. The AMD variant separately installs the Windows Radeon ROCm 7.2.1 SDK and matching PyTorch build; the NVIDIA variant uses the CUDA 13.0 index.
...
- **The NVIDIA build cannot use the GPU**: update the driver and confirm that it supports CUDA 13.0; use the CPU package if the problem remains.
