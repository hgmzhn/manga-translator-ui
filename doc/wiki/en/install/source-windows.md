...
- **NVIDIA**: the GPU group uses the `pytorch-cu126` index for `torch`/`torchvision`, `onnxruntime-gpu`, and `xformers`; the driver must support CUDA 12.6. If CUDA is insufficient, maintenance may recommend CPU.
