...
- 若启用 `use_custom_api_params`，上色器会自动把 `config/custom_api_params.json` 中 `colorizer` 分组的自定义参数并入对应后端请求体；例如：
...
- 若启用 `use_custom_api_params`,渲染器会自动把 `config/custom_api_params.json` 中 `render` 分组的自定义参数并入对应后端请求体
