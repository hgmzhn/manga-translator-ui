...
- **纯色气泡直接填色 (solid_fill_pure_bubbles)**：纯色背景气泡跳过修复模型
  - 逐个文本区域检测其所在气泡，若气泡内非文字背景接近纯色，直接用背景中位色填充气泡内部，不过修复模型
  - 优点：大量普通气泡无需过模型，速度更快，且无模型糊边
  - 默认关闭

- **逐块修复 (per_block_inpainting)**：使用优化蒙版对文本区域逐块裁窗修复，替代整页修复
  - 每个（未被纯色填充的）文本区域裁 1.7 倍窗口，直接裁取 mask refinement 生成的优化蒙版，反射补方后单独喂给修复模型再贴回
  - 每块补成正方形后长宽比为 1，不会进入长图切片流程
  - 通常会比整页修复更干净，避免整页长条掩码留下文字鬼影
  - 速度会比整页修复略慢（多次模型调用有固定开销）
  - 不属于任何文本区域的掩码碎块会被丢弃
  - 默认关闭；关闭时保持原有整页修复行为
