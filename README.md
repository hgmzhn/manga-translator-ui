...
#### `feature/align-distribute-buttons` 分支新增

- 📐 **PS 风格对齐/分布工具栏** — 8 按钮：左/中/右对齐 + 顶/中/底对齐 + 水平/垂直间距分布，支持选区或画布参照
- 🔲 **批量移动** — 多选文本框后拖拽任意一个，其余同步跟随
- 🧲 **智能间距吸附** — 拖拽时自动检测等距关系，显示双组辅助线和距离标签（阈值 8px）
- 📤 **导出全部** — 工具栏一键遍历导出所有已编辑图片
- 🖼️ **A/D 切换防黑闪** — 切换图片时保持上一张可见，消除黑屏闪烁
